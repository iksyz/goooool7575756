...
function buildSeoText({
    ...
    const p1 = `If you're searching for a ${quiz.title} experience that feels like real matchday pressure, this is the place to start. This football quiz is built around ${questionCount} quick questions designed to test recall, context, and instinct. You'll see themes connected to ${quiz.league || quiz.topic || quiz.category} and ${quiz.category}, but the real goal is deeper: to understand the moments, patterns, and details that define the game. With a focused time window (about ${minutes} minutes total), you get a fast, competitive session that rewards sharp thinking and confident decisions.`;
    const p2 = `This quiz is also crafted for SEO-friendly discoverability and for real fans who want meaningful practice. Instead of random prompts, the question set targets the topics football supporters actually search for: ${quiz.league || quiz.topic || quiz.category} trivia, classic moments, key records, famous fixtures, and the context behind iconic outcomes. If your interest is ${quiz.difficulty} difficulty, you'll enjoy the balance between accessible knowledge and the kind of details that separate casual viewers from true students of the sport.`;
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    const minutes = Math.max(2, Math.round((questionCount * 15) / 60));
    const seoLong =
    ...
    const structuredData = {
        ...
        "@graph": [
            ...
            },
            {
                ...
                inLanguage: "en",
                about: [
                    { "@type": "Thing", name: quiz.league || quiz.topic || quiz.title },
